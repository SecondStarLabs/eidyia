...
class LocalSchool::PageParser::Search
    ...
    def _parse_table_rows_into_hash
        @fragment.css('#tabSchooList').css('tbody').css('tr').each do |row|
            ...
            school_hash = {link_href: link_href, school_id: school_id, school_name: school_name, type: type, grades: grades, district: district_text, district_link: district_link, enrollment: enrollment_number, student_teacher_ratio: student_teacher_ratio_number, free_discounted_lunch_recipients: free_discounted_lunch_recipients_number, school_digger_rating: school_digger_rating_number, city: city_text}
            puts school_hash
            @schools << school_hash
        end
        @schools
